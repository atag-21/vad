...
def main_shangai():
    ...
    with torch.no_grad():
        ...
        ds = VideoDataset(root_dir=root_dir, dataset_name="shanghai")
...
def args_parser():
    ...
    p.add_argument("--batch_size", type=int, default=60, help="the batch size")
    p.add_argument("--gpu_id", type=int, default=0, help="gpu id to choose the gpu")
    p.add_argument(
        "--split_id", type=int, default=0, help="split id to choose the split"
    )
    p.add_argument("--split_n", type=int, default=1, help="n split to divide")
    p.add_argument(
        "--split_name", type=str, default="train", help="dataset split to process"
    )
    p.add_argument(
        "--reminders", action="store_true", default=False, help="reminder switch"
    )
    return p.parse_args()
